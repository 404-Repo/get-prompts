    ...
    async with httpx.AsyncClient() as client:
        ...
        for file in file_dir.iterdir():
            async with aiofiles.open(file, "rb") as f:
                ...
                data_stream.write(
                    msgpack.packb({"normalized_prompt": fake.sentence(), "data": file_data}, use_bin_type=True)
                )
        data_stream.seek(0)
        response = await client.post("http://localhost:8093/images/submit", files={"file": data_stream}, timeout=100000)
